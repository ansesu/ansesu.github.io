//Margin
const marginThreshold = { top: 10, right: 10, bottom: 25, left: 54 };
...
d3.csv("data/rt_maringa_data.csv")
  .then(function(data) {
	...
	const svgThreshold = d3.select('#numero-de-reproducao')
			               .append('svg')
			                .attr("class", "content")
    ...
    function mousemoveThreshold(event) {
	    ...
	    var dTrue = x0 - d0.Data > d1.Data - x0 ? d1 : d0; // if is true, d1, if is false d0
			xValue = dTrue.date;
		d3.select("#numero-de-reproducao .hover-line")
		   .attr("d", function() {
		...
		focusThreshold.select("#numero-de-reproducao .tooltip-text").text(function() { return "R(t) = " + dTrue.mean_r.toFixed(2); })
		focusThreshold.select("#numero-de-reproducao .tooltip-date").text(function() { return  formatAsDate(xValue); });			        
		focusThreshold.attr("transform", "translate(" + xScaleThreshold(xValue) + "," + yScaleThreshold(dTrue.mean_r) + ")");
		focusThreshold.select("circle")
		    		   .attr("opacity", "1");
		if (dTrue.date > medianThreshold) {
			focusThreshold.select("#numero-de-reproducao .tooltip-text")
					 	   .attr("y", heightThreshold*.11-yScaleThreshold(dTrue.mean_r))
					 	   .attr("text-anchor", "end")
					 	   .attr("x", -5);
			focusThreshold.select("#numero-de-reproducao .tooltip-date")
