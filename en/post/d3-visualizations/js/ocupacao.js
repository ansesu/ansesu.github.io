//Margin
const marginOcupacao = { top: 10, right: 0, bottom: 25, left: 48 };
...
function plotOcupacao(data_path, first, which, gOcupacao, id, svgOcupacao) {
  ...
  d3.csv(data_path)
    ...
    if (data_path == "data/ocupacao.csv") {
      var color_ = {'ICU': '#0077BB', 'Neonatal ICU': '#bbbbbb', 'Nursery': '#EE3377'}
      var colorOcupacao = d3.scaleOrdinal([color_[which]]); // https://personal.sron.nl/~pault/#sec:qualitative
      var x_tooltip = 10;
      var y_tooltip = 60 
    } else { 
      var color_ = {'ICU': '#33BBEE', 'Neonatal ICU': '#009988', 'Nursery': '#EE7733'}
      var colorOcupacao = d3.scaleOrdinal([color_[which]]); // https://personal.sron.nl/~pault/#sec:qualitative
      var x_tooltip = 10;
      var y_tooltip = 60   
    }
    ...
    data.forEach(function(d) {
      ...
      d.Nursery = parseFloat(d.Nursery.replace(',','.'));
      d.ICU = parseFloat(d.ICU.replace(',','.'));
      d["Neonatal ICU"] = parseFloat(d["Neonatal ICU"].replace(',','.'));           
      d.Nursery_total = parseInt(d.Nursery_total);
      d.ICU_total = parseInt(d.ICU_total);
      d["Neonatal ICU_total"] = parseInt(d["Neonatal ICU_total"]);
    });
...
plotOcupacao("data/ocupacao.csv", true, "ICU", gOcupacao1, "#ocupacao1", svgOcupacao1)
plotOcupacao("data/ocupacao.csv", true, "Neonatal ICU", gOcupacao2, "#ocupacao2", svgOcupacao2)
plotOcupacao("data/ocupacao.csv", true, "Nursery", gOcupacao3, "#ocupacao3", svgOcupacao3)